      ...
      <div className="py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 lg:gap-32 px-5 max-w-[1600px] mx-auto">
          {tabs.map((tab) => {
            ...
            return (
              <div
                key={tab.id}
                className={`flex items-center gap-5 rounded-2xl group cursor-pointer transition-all duration-500 ${
                  isActive || isHovered ? "scale-[1.02]" : "scale-100"
                }`}
                style={{
                  background: isActive || isHovered ? gradient : "transparent",
                }}
              ...
              >
                <div
                  className="lg:min-w-[140px] lg:min-h-[140px] flex items-center justify-center rounded-2xl bg-cover bg-center transition-opacity duration-500"
                  style={{ backgroundImage: `url(${tab.tab_image})` }}
                >
                  <span className="group-hover:opacity-100 opacity-0 text-transparent bg-clip-text bg-linear-to-r from-[#c20dd3f5] to-[#FFF] transition-opacity duration-500 ease-out text-2xl">
                    Click
                  </span>
                </div>
                <h4 className="text-2xl font-parkinsans text-primary-foreground">
                  {tab.tab_title}
                </h4>
              </div>
            );
        ...
        {activeTabData && (
          <div className="mt-20">
            ...
            {activeTabData.title === "Cybersecurity Solutions" && <OurCapabilities_Cybersecurity/>} 
            {activeTabData.title === "Cloud Infrastructure" && <OurCapabilities_CloudInfrastructure/>} 
            {/*  */}
            {activeTabData.title === "AI & Automation" && <TechnologyStack_AITransformation/>} 
            {activeTabData.title === "Cybersecurity Solutions" && <TechnologyStack_Cybersecurity/>} 
